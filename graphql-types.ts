...
export type ArticleListQuery = { allMarkdownRemark: { nodes: Array<(
      Pick<MarkdownRemark, 'id' | 'excerpt' | 'timeToRead'>
      & { frontmatter?: Maybe<Pick<MarkdownRemarkFrontmatter, 'date' | 'title' | 'slug' | 'tags'>> }
    )> } };

export type PostsByIdQueryVariables = Exact<{
  id: Scalars['String'];
}>;


export type PostsByIdQuery = { markdownRemark?: Maybe<(
